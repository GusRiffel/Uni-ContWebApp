...
function App() {
    ...
    const {isAuthenticated, signUserOut, user} = useAuth();
    ...
    const createNotification = (notifType, message) => {
        if (notifType === "success") {
            return NotificationManager.success(message, 'Logged In');
        }
        return NotificationManager.warning(message, 'Logged Out');
    }
    useEffect(() => {
        if (isAuthenticated) {
            ...
            navigate("/");
            setIsLoggedIn(true);
            createNotification("success", "You are now logged in")
        } else if (!isAuthenticated && isLoggedIn) {
            console.log(user);
            setIsLoggedIn(false);
            createNotification("", "You have now logged out")
        }
    }, [isAuthenticated])
    ...
    return (
        <div>
            <ThemeProvider theme={theme}>
                {!closeHeader ? <Header dropDown={openDropDown} dropDownState={setDropDownState} signOut={signUserOut}/> : null}
                {isAuthenticated ? <NotificationContainer/> : <NotificationContainer/>}
                <GlobalStyles/>
                <ClickWrapper onClick={() => setOpenDropDown(false)}>
                    <Routes>
                        <Route path="/" element={ <Protected authenticated={isAuthenticated}> <Dash days={15} checkins={checkins}/> </Protected>}/>
                        <Route path="/join" element={<Join/>}/>
                        <Route path="/login" element={<Login/>}/>
                        <Route path="/profile" element={ <Protected authenticated={isAuthenticated}><Profile/> </Protected>}/>
                        <Route path="/checkin" element={<Protected authenticated={isAuthenticated}><Checkin/> </Protected>}/>
